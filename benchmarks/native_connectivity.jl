...
function native_connectivity!(SUITE)
    ...
    SUITE["Connectivity"]["Graphs adapter connected"] = @benchmarkable Graphs.is_connected(
        GC.build_graph($connected, $num_vertices)
    ) seconds = 5
    SUITE["Connectivity"]["native connected"] = @benchmarkable GC._is_connected_graph_rep(
        $connected, $num_vertices
    ) seconds = 5
    SUITE["Connectivity"]["Graphs adapter disconnected"] = @benchmarkable Graphs.is_connected(
        GC.build_graph($disconnected, $num_vertices)
    ) seconds = 5
