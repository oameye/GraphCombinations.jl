...
"""
    allgraphs(n; connected=true)

Generate all unique topologies for a given vertex specification. `n[k]` is the number of vertices
of degree `k`. Connected graphs are generated by default; set `connected=false` to retain
disconnected topologies.

Returns tuples `(edges, S)`, where `edges` is a `Vector{Pair{Int,Int}}` and `S::BigInt` is the exact
symmetry denominator. Each edge `a => b` is stored with `a ≤ b`; self-loops are allowed.
...
"""
    combinatoric_factor(n)

Compute the exact combinatorial normalization used to obtain graph symmetry denominators.
...
external and remain fixed. The second product counts permutations of edge endpoints at each vertex.
Returns a `BigInt`.
"""
