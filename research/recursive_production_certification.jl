...
function recursive_graph_from_mask(n::Int, mask::UInt64)
    edges = Pair{Int,Int}[]
    bit_index = 0
...
function recursive_restricted_growth_colorings(n::Int)
    n == 0 && return [Int[]]
...
function assert_production_recursive_matches!(
    label::String,
    ...
    recursive_workspace::RecursiveGC.PackedRecursiveStabilizerWorkspace;
...
function certify_recursive_n3_exhaustive()
    n = 3
    colorings = recursive_restricted_growth_colorings(n)
...
function certify_recursive_n4_uniform_exhaustive()
    n = 4
    colors = ones(Int, n)
    base_workspace = GC.PackedDirectedCanonicalizationWorkspace(n)
    base_buffer = GC.DirectedCanonicalizationBuffer(n)
...
function certify_recursive_n4_color_partitions_sample()
    n = 4
    colorings = recursive_restricted_growth_colorings(n)
    base_workspace = GC.PackedDirectedCanonicalizationWorkspace(n)
    base_buffer = GC.DirectedCanonicalizationBuffer(n)
...
@inline function recursive_xorshift64(value::UInt64)::UInt64
    value ⊻= value << 13
    value ⊻= value >> 7
    value ⊻= value << 17
    return value
...
function certify_recursive_n5_deterministic_sample()
    ...
    for sample in 1:4096
        state = recursive_xorshift64(state)
        graph_mask = state & mask_limit
        graph = recursive_graph_from_mask(n, graph_mask)
        for colors in colorings
            cases += 1
            assert_production_recursive_matches!(
...
total = 0
total += certify_recursive_n3_exhaustive()
total += certify_recursive_n4_uniform_exhaustive()
total += certify_recursive_n4_color_partitions_sample()
total += certify_recursive_n5_deterministic_sample()
