...
function benchmark_fixture(name::String, graph::GC.DirectedGCGraph, colors::Vector{Int})
    ...
    for vertex in 1:n
        GC.canonical_rank(buffer, vertex) == GC.canonical_rank(expected, vertex) ||
    ...
    println(
        ...
        "|",
        metric(workspace, :root_orbit_skips),
        "|",
        metric(workspace, :root_orbit_merges),
        "|",
        metric(workspace, :root_automorphisms),
    )
    return nothing
end
