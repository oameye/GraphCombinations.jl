...
function compatible_schedule_fragments!(
    workspace::ActiveWordWorkspace, num_fragments::Int, tail::Int
)::Int
    largest_fragment = 1
    ...
    @inbounds for fragment in 1:num_fragments
        ...
        tail <= length(workspace.queue) ||
            error("compatible splitter queue capacity exceeded")
        workspace.queue[tail] = workspace.fragment_masks[fragment]
    end
    return tail
end

function compatible_active_refine!(
    ...
    while head <= tail && num_cells < n
        ...
        @inbounds for cell in 1:num_cells
            ...
            tail = compatible_schedule_fragments!(workspace, num_fragments, tail)
        end
...
function compatible_active_search!(
    ...
    word.search_nodes += 1
    compatible_active_refine!(workspace, depth, n, seed_mask)
...
function canonicalize_compatible_active_word!(
    ...
    n <= length(word.colors) ||
        throw(DimensionMismatch("compatible word workspace too small"))
    ...
    compatible_active_search!(workspace, graph, 1, 1, UInt64(0))
    word.has_best || error("compatible word canonical search produced no candidate")
    write_word_buffer!(buffer, word, graph)
    return buffer
end

@testset "compatible scheduled splitter exact exhaustive 3-vertex simple digraphs" begin
    ...
    compatible_workspace = ActiveWordWorkspace(3)
    compatible_buffer = GC.DirectedCanonicalizationBuffer(3)
    ...
    for mask in 0:(2 ^ 9 - 1)
        ...
        for colors in colorings
            ...
            canonicalize_compatible_active_word!(
                compatible_buffer, compatible_workspace, graph, colors
            )
            assert_same_result(baseline_buffer, compatible_buffer, 3)
            @test GC.canonical_graph(compatible_buffer) == GC.canonical_graph(word_buffer)
        end
    end
end

function benchmark_compatible_fixture(
    ...
    compatible_workspace = ActiveWordWorkspace(n)
    compatible_buffer = GC.DirectedCanonicalizationBuffer(n)
    ...
    assert_same_result(baseline_buffer, compatible_buffer, n)
    @test GC.canonical_graph(compatible_buffer) == GC.canonical_graph(word_buffer)
    @test GC.canonical_graph(compatible_buffer) == GC.canonical_graph(active_buffer)

    load_word_rows!(word_workspace, graph)
    load_word_rows!(active_workspace.word, graph)
    load_word_rows!(compatible_workspace.word, graph)
    ...
    compatible_time = @belapsed canonicalize_compatible_active_word!(
        $compatible_buffer, $compatible_workspace, $graph, $colors; load_rows=false
    ) samples=101 evals=1
    compatible_full_time = @belapsed canonicalize_compatible_active_word!(
        $compatible_buffer, $compatible_workspace, $graph, $colors
    ) samples=101 evals=1
    ...
    compatible_splitters = compatible_workspace.splitter_visits
    compatible_alloc = @allocated canonicalize_compatible_active_word!(
        compatible_buffer, compatible_workspace, graph, colors
    )
    ...
    return println(
        join(
            (
                ...
                "compatible=$(round(1e6 * compatible_time; digits=2)) us",
                "compatible-full=$(round(1e6 * compatible_full_time; digits=2)) us",
                "compatible/base=$(round(compatible_full_time / baseline_time; digits=3))x",
                "compatible/word=$(round(compatible_time / word_time; digits=3))x",
                "compatible/active=$(round(compatible_time / active_time; digits=3))x",
                "alloc=$(compatible_alloc)",
                "nodes=$(compatible_workspace.word.search_nodes)",
                "leaves=$(compatible_workspace.word.search_leaves)",
                "splitters=$(naive_splitters)->$(compatible_splitters)",
            ),
            " | ",
        ),
    )
...
println("\ncompatibility-preserving splitter scheduling benchmark")
for (name, fixture) in (
    ...
    benchmark_compatible_fixture(name, graph, colors)
end
