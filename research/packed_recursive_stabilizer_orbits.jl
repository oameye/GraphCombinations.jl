...
mutable struct RecursiveStabilizerWorkspace
...
function RecursiveStabilizerWorkspace(capacity::Integer)
    ...
    return RecursiveStabilizerWorkspace(
...
@inline function recursive_slot(
    candidate::RecursiveStabilizerWorkspace, depth::Int, vertex::Int
)::Int
...
function recursive_orbit_union!(
    candidate::RecursiveStabilizerWorkspace, depth::Int, left::Int, right::Int
)::Nothing
...
function recursive_initialize_node!(
    candidate::RecursiveStabilizerWorkspace,
...
function recursive_candidate_explored(
    candidate::RecursiveStabilizerWorkspace, depth::Int, chosen_vertex::Int
)::Bool
...
function recursive_copy_leaf!(
    candidate::RecursiveStabilizerWorkspace, graph::GC.DirectedGCGraph, depth::Int
)::Nothing
    workspace = candidate.packed.workspace
    n = graph.num_vertices
    @inbounds for vertex in 1:n
...
function canonicalize_recursive_stabilizers!(
    ...
    GC._prepare_packed_directed_rows!(packed, graph) ||
        error("recursive stabilizer research requires a simple directed graph with n <= 64")
