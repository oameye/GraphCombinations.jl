...
using GraphCombinations: Edge, GraphRep
...
@testset "Graph Reduction (reduce_isomorphic_graphs)" begin
    ...
    @test ReferenceGC.reduce_isomorphic_graphs(GraphRep[], 1:0) == [] # Use 1:0 for empty range

    # Test case 2: Single graph, no internal vertices
    graph1 = [Edge(1, 2)]
    @test ReferenceGC.reduce_isomorphic_graphs([graph1], 1:0) == [graph1 => 1] # Use 1:0 for empty range, expect Pair
    ...
    @test ReferenceGC.reduce_isomorphic_graphs([graph2], 3:4) ==
        [expected_graph2_sorted => 1] # Expect Pair

    # Test case 3: Two identical graphs, no internal vertices
    graph3 = [Edge(1, 2), Edge(3, 4)]
    graph3_sorted = sort_graph_props_test(graph3)
    @test ReferenceGC.reduce_isomorphic_graphs([graph3, graph3], 1:0) ==
        [graph3_sorted => 2] # Use 1:0, expect Pair
    ...
    @test sort_reduce_results(
        ReferenceGC.reduce_isomorphic_graphs([graph4a, graph4b], 1:0)
    ) == sort_reduce_results(expected4) # Use 1:0
    @test sort_reduce_results(
        ReferenceGC.reduce_isomorphic_graphs([graph4b, graph4a], 1:0)
    ...
    result5 = ReferenceGC.reduce_isomorphic_graphs([g5a, g5b], internal5)
    @test length(result5) == 1
    @test result5[1] == (canonical5 => 2) # Check the Pair directly
    # Test with duplicates of one form
    result5_dup = ReferenceGC.reduce_isomorphic_graphs([g5a, g5a, g5b], internal5)
    ...
    result6 = ReferenceGC.reduce_isomorphic_graphs([g6a, g6b], internal6)
    ...
    result7 = ReferenceGC.reduce_isomorphic_graphs([g7a, g7b, g7c], internal7)
